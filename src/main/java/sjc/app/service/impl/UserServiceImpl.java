...
public class UserServiceImpl implements UserService {
	...
	public RegisterUserVO getUserByName(String userName) {
		...
		RegisterUserVO registerUserVO = new RegisterUserVO(registerUser.getId(),registerUser.getName(),registerUser.getLastName(),registerUser.getPassword(),registerUser.getLogin());
	...
	//	System.out.println(registerUsers.get(0).getName());
		...
		for (RegisterUser user:registerUsers) {
			...
			for (RegisterUser friend  :user.getFriends()) {
				RegisterUserVO friendVO=new RegisterUserVO(friend.getId(),friend.getName(),friend.getLastName(),friend.getPassword(),friend.getLogin());
			    friendsVO.add(friendVO);
			}
			RegisterUserVO registerUserVO =new RegisterUserVO(user.getId(),user.getName(),user.getLastName(),user.getPassword(),user.getLogin(),friendsVO);
